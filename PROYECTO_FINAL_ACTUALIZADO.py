...
def normalizar_zona(zona):
    ...
    # Quita espacios de sobra al inicio/final y pasa todo a minusculas
    # para que "Escuela", "escuela " y " ESCUELA" se traten como la misma zona
    return zona.strip().lower()
...
def agregar_ruta(zona1, zona2, distancia):
    ...
    zona1 = normalizar_zona(zona1)
    zona2 = normalizar_zona(zona2)
...
def bfs_existe_camino(inicio, destino):
    ...
    inicio = normalizar_zona(inicio)
    destino = normalizar_zona(destino)

    print(f"\n--- BFS: {inicio} -> {destino} ---")

    if inicio not in grafo_zonas or destino not in grafo_zonas:
        print("Una de las zonas no existe en el grafo.")
        print("Zonas disponibles:", list(grafo_zonas.keys()))
        return False, []
...
def mostrar_menu():
    ...
    print("9. Mostrar recorrido Preorden")
    print("10. Mostrar recorrido Inorden")
    print("11. Mostrar recorrido Postorden")
    print("12. Buscar misión en el árbol")
    print("13. Búsqueda lineal de dron")
    print("14. Búsqueda lineal de misión")
    print("15. Búsqueda binaria de dron")
    print("16. Agregar zona")
    print("17. Agregar ruta entre zonas")
    print("18. Mostrar grafo")
    print("19. Verificar camino")
    print("20. Calcular ruta minima ")
    print("21. Simulacion completa de rescate")
    print("22. Salir")
...
def ejecutar_menu():
    ...
    while True:
        ...
        if opcion == "1":
           ...
           lista = quickSort_distancia(cola_misiones)
           print("\nMisiones ordenadas por distancia:")
           for m in lista:
            print(f"{m['codigo']} - Distancia: {m['distancia']} km")
            ...
            print("\nPaso 2: Agregar a la cola")
            agregar_mision_a_cola(codigo, zona, tipo, prioridad, personas, distancia)

            print("\nPaso 3: Insertar en el BST")
            raiz_misiones = actualizar_arbol_misiones()

           
            print("\nPaso 4: Ordenar misiones por prioridad")
            burbuja_prioridad(cola_misiones)

        
            print("\nPaso 5: Buscar dron disponible")
            dron = buscar_dron_disponible()

        
            print("\nPaso 6: Verificar ruta (BFS)")

            if len(grafo_zonas) == 0:

                print("El grafo esta vacio. Agrega zonas y rutas primero (opciones 15 y 16).")
                continue

            print("Zonas disponibles en el grafo:", list(grafo_zonas.keys()))

           
            zona_origen = normalizar_zona(input("Zona de ORIGEN (desde donde sale el dron): "))

            while zona_origen not in grafo_zonas:

                print(f"'{zona_origen}' no existe en el grafo.")
                print("Zonas disponibles:", list(grafo_zonas.keys()))
                zona_origen = normalizar_zona(input("Escribe de nuevo la zona de ORIGEN: "))

           
            zona_destino = normalizar_zona(zona)

            while zona_destino not in grafo_zonas:

                print(f"'{zona_destino}' no existe en el grafo.")
                print("Zonas disponibles:", list(grafo_zonas.keys()))
                zona_destino = normalizar_zona(input("Escribe de nuevo la zona DESTINO (de la mision): "))

            existe, camino = bfs_existe_camino(zona_origen, zona_destino)

            if existe:

        
                print("\nPaso 7: Calcular ruta minima (Dijkstra)")
                ruta, distancia_total = dijkstra(zona_origen, zona_destino)

            
                print("\nPaso 8: Asignar dron")
                nombre_dron = dron["codigo"] if dron else "---"
                print(f"Dron {nombre_dron} enviado por: {' -> '.join(ruta)}")
